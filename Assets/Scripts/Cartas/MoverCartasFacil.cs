...
public class MoverCartasFacil : MonoBehaviour
{
    //Arrastrar y soltar

    private MemoryFacil memory;
    ...
    IEnumerator ExecuteAfterTime(float time)
    {
            ...
            memory = FindObjectOfType<MemoryFacil>();
    ...
    private void OnMouseDown()
    {
        if (!locked && lockedPausa == true)
        {
            ...
            if (posicionActual == 1)
            {
                ...
                {
                    MemoryFacil.numeroOmisiones = MemoryFacil.numeroOmisiones + 1;
                    contador--;
                }
            }
            if (posicionActual == 2)
    ...
    private void OnMouseUp()
    {
        if (lockedPausa == true)
        {
             ...
             this.name == deckComparativo[0] && lockPosicion0 == false)
            {
                ...
                lockPosicion0 = true;
                contador++;
                MemoryFacil.numeroAciertos = MemoryFacil.numeroAciertos + 1;
                posicionActual = 1;
            }
                 ...
                 Mathf.Abs(transform.position.y - bottomPos[0].transform.position.y) <= 2f && lockPosicion0 == false)
            {
                transform.position = new Vector2(bottomPos[0].transform.position.x, bottomPos[0].transform.position.y);
                lockPosicion0 = true;
                MemoryFacil.numeroErrores = MemoryFacil.numeroErrores + 1;
                posicionActual = 1;
    ...
    void Update()
    {
       ...
       if(MemoryFacil.desbloquearCartas == 0)
        {
            locked = true;
        }
        else if(MemoryFacil.desbloquearCartas == 1)
        {
            locked = false;
        }

        if (lockPosicion0 == true && lockPosicion1 == true && lockPosicion2 == true 
            && contador != 3)
        {

            FindObjectOfType<MemoryFacil>().volverAIntentar();
            int i = 0;
            if (deckComparativo.Count != 0)
            {
                foreach (string s in deckComparativo)

...
public class Palabras : MonoBehaviour
{
    ...
    public static string[] images = new string[] { "barcod", "circoq", "discob", "pandab", "lápizq", "indiob", "árbold", "librod", "nieveb", "autosv" };
    ...
    void Start()
    {
        ...
        print(" CEDULA: "+dontDestroy.cedulaUsuario);
    ...
    public void terminarJuego()
    {
        ...
        FindObjectOfType<DBManager>().BtnGuardarPartidaNueva(dontDestroy.cedulaUsuario, "Palabras", "Inicial", numeroAciertos,
    ...
    IEnumerator nuevaRonda(float time)
    {
        ...
        if (ronda < 4)
        {
            ...
            UpdateLetra[] letras = FindObjectsOfType<UpdateLetra>();
            int i = 0;
            foreach (UpdateLetra card in letras)
            {
                if (i < 18)
                {
            ...
            UpdateImagen[] imagenes = FindObjectsOfType<UpdateImagen>();
            int j = 0;
            foreach (UpdateImagen card in imagenes)
            {
                if (j < 1)
                {
                    Destroy(card.gameObject);
            ...
            FindObjectOfType<Palabras>().EmpezarJuego();
            FindObjectOfType<UpdateLetra>().CrearLetras();
            FindObjectOfType<UpdateImagen>().CrearImagen();
            FindObjectOfType<MoverLetras>().IniciarMovimientos();
            detenerLoop = false;
        }
    ...
    void AgregarLetrasPalabraImagen(List<string> deck)
    {
        ...
        {
            case 0:
                for(int i=0; i<6; i++)                                  //cambiar en dificultad
                {

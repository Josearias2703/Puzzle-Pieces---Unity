...
public class PalabrasDificil : MonoBehaviour
{
    ...
    public static string[] images = new string[] { "cinturóns", "cuadernoq", "cuchilloy", "guitarraj", "ladrilloy", "espátulaq", "anteojosg", "estrellay", "libreríab"};
    ...
    void Update()
    {
        if (MoverLetrasDificil.contador == 8)         
        {
            if (ronda == 4)
            {
                terminarJuego();
    ...
    public void terminarJuego()
    {
        ...
        FindObjectOfType<DBManager>().BtnGuardarPartidaNueva(dontDestroy.cedulaUsuario, "Palabras", "Experto", numeroAciertos,
    ...
    IEnumerator nuevaRonda(float time)
    {
        ...
        if (ronda < 4)
        {
            ...
            UpdateLetraDificil[] letras = FindObjectsOfType<UpdateLetraDificil>();
            int i = 0;
            foreach (UpdateLetraDificil card in letras)
            {
                if (i < 18)
                {
            ...
            UpdateImagenDificil[] imagenes = FindObjectsOfType<UpdateImagenDificil>();
            int j = 0;
            foreach (UpdateImagenDificil card in imagenes)
            {
                if (j < 1)
                {
                    Destroy(card.gameObject);
            ...
            FindObjectOfType<PalabrasDificil>().EmpezarJuego();
            FindObjectOfType<UpdateLetraDificil>().CrearLetras();
            FindObjectOfType<UpdateImagenDificil>().CrearImagen();
            FindObjectOfType<MoverLetrasDificil>().IniciarMovimientos();
            detenerLoop = false;
        }
    ...
    void AgregarLetrasPalabraImagen(List<string> deck)
    {
        ...
        {
            case 0:
                for (int i = 0; i < 9; i++)                                  //cambiar en dificultad
                {
